...
def check_title_order(found_titles, require_titles):
    ...
    explanation.extend([f"제 12조 <{title}>의 순서/위치가 잘못되었습니다." for title in mismatched])

    missing_titles = [title for title in require_titles if title not in found_titles]
    incorrect_titles.extend(missing_titles)
    explanation.extend([f"제 12조 <{title}>가 있어야하지만 빠졌습니다." for title in missing_titles])
    ...
    explanation.extend([f"제 12조 <{title}>의 순서/위치가 잘못되었습니다." for title in misordered_titles])
...
def check_title_content(documents, title_list, found_titles, require_titles):
    ...
    except_title = extract_top_titles(title_list)
    ...
    for title in find_titles:
        ...
        if len(paragraph)!=0:
            ...
            explanation.append(f"제 12조 <{title}>에 기술 또는 '내용 없음'을 작성해야 합니다.")
    ...
    for title in except_title:
        ...
        if len(paragraph)==0:
            ...
            incorrect_titles.append(title)
            explanation.append(f"제 12조 <{title}>에 내용을 기술하지 말아야합니다.")
...
def KDS_configure_12(hwp):
    '''
    제 12조를 검사하고 잘못된 문장을 모음
...
def KDS_312(hwp):
    ...
    :return: 문서가 하이라이팅 됌
    '''
    incorrect_titles, explanation = KDS_configure_12(hwp)
    if incorrect_titles == explanation == False:
        print(f"제 12조 <1. 일반사항>을 추가하셔야 오류 검사가 가능합니다.")
        return 0
...
#     file_path = r'../data/KDS115005.hwp'
#     hwp = Hwp()
#     hwp.open(file_path)
#     KDS_312(hwp)
