...
def K418_132_2_0(hwp):
    ...
    예) 건축물의 에너지절약설계기준(제1조①항1호)   -> 이런건 특징점이 없어서 못한다. 1.3.2에 없 이걸 풀게 하려면 관련 기준을 모두 모아서 니들이 줘야 한다.
    ...
    #             hwp.find(src=f'{code}')
    #             hwp.insert_memo(
    #                 '“1.3.2 관련기준”에 적합하지 않은 기준입니다. 건설기준 코드의 경우 “코드+코드번호+기준명”을으로 표기하고, 건설기준 코드가 아닌 관련 기준일 경우“기준명+(부처명)”으로 표기해 주십시오.')
    # print(code182)
